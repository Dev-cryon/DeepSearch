...
const extractLinks = (body:string, url:string, maxLinks:number, searchTerms?:string[]) => {
	const scoredLinks = [...body.matchAll(/<a\s+[^>]*?href="([^"]+)"[^>]*>((?:\n|.)*?)<\/a>/g)]
		.map((match, index) => ({
			index,
	...
	// Performance optimization: O(n) deduplication using Set instead of O(n^2) Array.prototype.find
	const seenLinks = new Set<string>();
	const uniqueLinks: [string, string][] = [];

	for (const { label, link } of scoredLinks) {
